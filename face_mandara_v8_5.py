...
from objects import similar_window_v4
    ...
    with Manager() as manager:
        ...
        while True:
            ...
            try:
                ...
                print("something occured in making all_images")
            ...
            try:
                ...
                print("something occured in ready rect manager")
            ...
            for i in range(len(face_rect_manager)):
                ...
                for j in range(len(all_images[i])):
                    sw = similar_window_v4.SimilarWindow(distance=distance_no1, \
            ...
            while True:
                ...
                for i in range(len(similar_windows)):
                    for j in range(len(similar_windows[i])):
                        ...
                        face_frame_width = face_frames[i].right - face_frames[i].left
                        face_frame_height = face_frames[i].top - face_frames[i].bottom
                        if t > end_frame_num:
                            t = end_frame_num
                        if  j == 0:
                            # 時間　初期位置、移動量、終了時間、イージングタイプ
                            x = easing.easing(t, face_frame_center_x-80, 0, end_frame_num, easing_type)
                            y = easing.easing(t, face_frame_center_y-80, 250, end_frame_num, easing_type)
                        elif j == 1:
                            x = easing.easing(t, face_frame_center_x-80, 300, end_frame_num, easing_type)
                            y = easing.easing(t, face_frame_center_y-80, 0, end_frame_num, easing_type)
                        elif j == 2:
                            x = easing.easing(t, face_frame_center_x-80, 300, end_frame_num, easing_type)
                            y = easing.easing(t, face_frame_center_y-80, -250, end_frame_num, easing_type)
                        elif j == 3:
                            ...
                            x = easing.easing(t, face_frame_center_x-80, similar_windows[i][j].easing_param[0], end_frame_num, easing_type)
                            y = easing.easing(t, face_frame_center_y-80, similar_windows[i][j].easing_param[1], end_frame_num, easing_type)



                        # # 上部に移動
                        # elif j < 20:
                        #     x = easing.easing(t, face_frame_center_x-80, random.randint(-(face_frame_center_x-80), frame.width-(face_frame_center_x-80)), end_frame_num, easing_type)
                        #     y = easing.easing(t, face_frame_center_y-80, random.randint(-(face_frame_center_y-80), face_frame_center_y), end_frame_num, easing_type)
                        # # 下部に移動
                        # elif j < 30:
                        #     x = easing.easing(t, face_frame_center_x-80, random.randint(-(face_frame_center_x-80), frame.width-(face_frame_center_x-80)), end_frame_num, easing_type)
                        #     y = easing.easing(t, face_frame_center_y-80, random.randint(face_frame_center_y, frame.height-(face_frame_center_y-80)), end_frame_num, easing_type)
                        # # 左部に移動
                        # elif j < 40:
                        #     x = easing.easing(t, face_frame_center_x-80, random.randint(-(face_frame_center_x-80), face_frame_center_x), end_frame_num, easing_type)
                        #     y = easing.easing(t, face_frame_center_y-80, random.randint(-(face_frame_center_y-80), frame.height-(face_frame_center_y-80)), end_frame_num, easing_type)
                        # # 右部に移動
                        # else:
                        #     x = easing.easing(t, face_frame_center_x-80, random.randint(face_frame_center_x, frame.width), end_frame_num, easing_type)
                        #     y = easing.easing(t, face_frame_center_y-80, random.randint(-(face_frame_center_y-80), frame.height-(face_frame_center_y-80)), end_frame_num, easing_type)
                        x = int(x)
                        y = int(y)
                        print("xy",x, y)
